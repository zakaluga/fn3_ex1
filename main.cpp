...
//general block ncurses windows start
  ...
  void menu_item_delete(MENU* selected_menu, ITEM** selected_items, int arrlength, WINDOW* main_window) {
  ...
  int menu_window(const char* arr[], int arrlength, WINDOW* main_window, MENU* selected_menu, ITEM** selected_items) { 
    int ch;
    int sizeX = 20;
    int sizeY = 10;
    ...
    /* set menu window */
    set_menu_win(selected_menu, main_window);
    /*set menu sub window */
    set_menu_sub(selected_menu, derwin(main_window, sizeY-(sizeY/5), sizeX-(sizeX/5), sizeY/5, sizeX/5));
  ...
  int menu_window() {      
        const char *main_menu_choices[] = {"Append item", "Get info", "Delete item", "Exit"}, 
          ...
          *ginfo_menu_choices[] = {"Get all objects info", "Get books info", "Get furniture info", "Get computers info", "Get monitors info", "Get other info"}, 
          *del_menu_choices[] = {"Delete all objects", "Delete book", "Delete furniture", "Delete computer", "Delete monitor", "Delete other"};
        MENU 
      ...
      switch(ch) {
        ...
        case '\n':
            if (current_item(main_menu) == main_menu_items[0]) {
              menu_window(append_menu_choices, 5, main_window, append_menu, append_menu_items);
              break;
            }
            else if (current_item(main_menu) == main_menu_items[1]) {
              menu_window(ginfo_menu_choices, 6, main_window, ginfo_menu, ginfo_menu_items);
              break;
            }
            else if (current_item(main_menu) == main_menu_items[2]) {
              menu_window(del_menu_choices, 6, main_window, del_menu, del_menu_items);
              break;
            }
