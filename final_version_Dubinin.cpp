...
void Main_Menu_Print()
{
    ...
    cout << "2. Show everything that is in inventory" << endl;
...
namespace Computer
{
    ...
    public:
        ...
        static void Map_Computer_Deleting(std::list<Computers> &List_computer) // ищет айди в мапе, выполняет удаление по айди
        {
            ...
            if (a == "Computer")
            {
                if (!List_computer.empty())
                {
                    auto to_delete = List_computer.begin();
    ...
    {
        return out << "object's ID: " << computer.ID << std::endl
...
namespace Custom
{
    ...
    public:
        ...
        map<string, string> Get_object() const
        {
            return object;
        };
        void Set_object(map<string, string> set_object)
        {
            object = set_object;
        };
            ...
            if (a == "Custom")
            {
                for (auto q = List_custom.begin(); q != List_custom.end(); q++)
                {
                    ...
                    if (requested_ID == temporary_variable.ID)
                    {
                        ...
                        case 2:
                        {
                            cout << "which exact characteristic you want to change" << endl;
                            string _name;
                            cin >> _name;
                            cout << "Enter new value: ";
                            string _value;
                            cin >> _value;
                            map<string, string> mapin = peremennaya.Get_object();
                            mapin[_name] = _value;
                            peremennaya.Set_object(mapin);
                            cout << "Updated element: " << endl;
                            cout << peremennaya;
                        }
                        break;
        ...
        static void Map_Custom_Deleting(std::list<Customs> &List_custom) // ищет айди в мапе, выполняет удаление по айди
        {
            std::string requested_ID;
            cout << "Enter ID of object you want to delete: ";
            cin >> requested_ID;
            ...
            if (a == "Custom")
            {
                if (!List_custom.empty())
                {
                    auto to_delete = List_custom.begin();

                    for (auto i = List_custom.begin(); i != List_custom.end(); i++)
                    {
                        Customs temporary_variable = *i;
                        if (requested_ID == temporary_variable.ID)
                        {
                            to_delete = i;
                        }
                    }
                    List_custom.erase(to_delete);
                    Delete_Element_ID(requested_ID);
                    cout << "Object with ID: '" << requested_ID << "' has been removed" << endl;
                    Empty_Print();
        ...
        static void Custom_Switch(std::list<Customs> &List_custom)
        {
            ...
            while (second_choise != 4)
            {
                ...
                case 2: 
                {
                    Map_Custom_Deleting(List_custom);
                    Custom_Menu_Print();
                    cin >> second_choise;
                }
                break;
    ...
    {
        out << "object's ID: " << custom.Get_ID() << std::endl
            << "object's name: " << custom.Get_name() << std::endl;
        for (auto &[key, value] : custom.Get_object())
        {
...
void Print_Everythig(list<Custom::Customs> &List_custom, list<Computer::Computers> &List_computer)
{
    if (List_custom.empty() && List_computer.empty())
    {
        Empty_Print();
        cout << "There's nothing in this list" << endl;
    }
...
int main()
{
    ...
    while (main_choise_2 != 4)
    {
        ...
        case 2:
        {
            Print_Everythig(List_custom, List_computer);
        }
        break;
